...
(function (L08a) {
    ...
    function mathX() {
        let maxX = 735;
        let ranX = Math.floor(Math.random() * (maxX - min + 1)) + min;
        return ranX;
    }
    function mathY() {
        let maxY = 400;
        let ranY = Math.floor(Math.random() * (maxY - min + 1)) + min;
        return ranY;
    }
    function handleDance(_event) {
        let btt = document.getElementById("button1");
